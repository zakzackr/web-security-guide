...
### 原因
- 利用者によってアップロードされたスクリプトファイルがWebサーバの公開ディレクトリに保存され、利用者がそれらを直接呼び出すことで、サーバ上でプログラムが実行される
...
* モダンなWebアプリでは、画像ファイルなどの静的コンテンツはオブジェクトストレージサービスやCDNから配信されることが一般的である。その場合の対策方法については要確認。
