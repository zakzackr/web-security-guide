...
具体的には、攻撃者がシリアライズされたオブジェクトAをCookieやhiddenパラメータ経由で注入し、それがデシリアライズされることで、意図しない形でオブジェクトAがアプリケーション内に生成される。それにより、クラスAに`__destruct()`などのマジックメソッドが存在する場合、それらが実行されることで被害が発生する。
　 
