...
### 対策
基本的には、Session ID のみを Cookie に保管し、ユーザー ID や権限情報は、サーバー側のメモリやデータベースで管理する。**CookieをHttpOnlyにすることでJavaScript経由でのCookieの書き換えを防ぐことができるが、ブラウザの検証ツールから直接Cookieの書き換えができてしまうので、Session ID以外はサーバー側で管理。**
